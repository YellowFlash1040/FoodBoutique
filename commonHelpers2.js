import{b as tt,g as L,e as $,s as f,f as et}from"./assets/footer-8a0d29db.js";import{a as B}from"./assets/vendor-bb737cb9.js";const M=document.getElementById("categories");async function st(){const t=await tt();nt(t)}function nt(t){const e=[];for(const s of t){const a=document.createElement("li");a.className="categories-item",a.value=s;const o=document.createElement("button");o.type="button",o.className="categories-btn",o.setAttribute("data-value",s),o.innerText=s.replaceAll("_"," "),a.appendChild(o),e.push(a)}M.append(...e),M.innerHTML+=`
    <li class="categories-item">
      <button class="categories-btn" data-value="">
        Show all
      </button>
    </li>
  `}const ot=document.getElementById("keyword-input"),S=u();S?ot.value=S.keyword:l({keyword:null,category:null,page:1,limit:6});function l(t){localStorage.setItem("filters",JSON.stringify(t))}function u(){return JSON.parse(localStorage.getItem("filters"))}const v=document.getElementById("to-the-start"),h=document.getElementById("previous"),A=document.getElementById("pages-list"),b=document.getElementById("next"),y=document.getElementById("to-the-end");let _=null,m=null,r=1;function at(t){r=t}function g(){return _}function ct(t){_=t}function it(t){const e=[],s=Math.min(g(),m),a=s+r-1,o=t.page-a;o>0&&(r+=o);for(let n=r;n<s+r;n++){let d=function(){l({...t,page:n}),p()};const c=document.createElement("li");c.className="pages-item";const i=document.createElement("button");i.type="button",i.className="circle-btn number-btn "+(t.page===n?"current":""),i.innerHTML=n,i.addEventListener("click",d),c.appendChild(i),e.push(c)}A.innerHTML="",A.append(...e)}function q(){v.classList.add("disabled"),h.classList.add("disabled"),h.removeEventListener("click",H),v.removeEventListener("click",D)}function rt(){v.classList.remove("disabled"),h.classList.remove("disabled"),h.addEventListener("click",H),v.addEventListener("click",D)}function z(){y.classList.add("disabled"),b.classList.add("disabled"),b.removeEventListener("click",W),y.removeEventListener("click",O)}function lt(){y.classList.remove("disabled"),b.classList.remove("disabled"),b.addEventListener("click",W),y.addEventListener("click",O)}function H(){const t=u();t.page--,l(t),r-1===t.page&&r--,t.page===1&&q(),p()}function W(){const t=u();t.page++,l(t);const e=Math.min(g(),m);r+e===t.page&&r++,t.page===g()&&z(),p()}function D(){const t=u();t.page=1,l(t),r=1,p()}function O(){const t=u();t.page=g(),l(t);const e=Math.min(g(),m);r=t.page-e+1,p()}function R(){let t=window.innerWidth;t>=1440?(l({...u(),limit:9}),m=5):t>=768?(l({...u(),limit:8}),m=5):(l({...u(),limit:6}),m=3),p()}const ut="/images/svg/icons.svg#icon-shopping-cart",dt="/images/svg/icons.svg#icon-close";let w;async function C(t){const e=await vt(t),s=ft(e,t);pt(s)}function pt(t){w=basicLightbox.create(t)}function gt(t){const e=t.currentTarget.dataset.id;$(e),f(),mt()}let P;function mt(){const t=P.querySelector("span");t.textContent="Remove from"}function ft({name:t,category:e,size:s,popularity:a,desc:o,price:n,img:c},i){e=e.replace(/_/g," ");let d;return L().includes(i)?d="Remove from":d="Add to",`
    ...
    `}async function vt(t){const e=`https://food-boutique.b.goit.study/api/products/${t}`;return(await B.get(e)).data}function E(){w.show();const t=".detailed-product-info-modal-window .close-icon";document.querySelector(t).addEventListener("click",w.close),P=document.querySelector(".detailed-product-info-modal-window .addToCart-button"),P.addEventListener("click",gt)}const N="/FoodBoutique/images/svg/icons.svg",ht=document.getElementById("keyword-input"),k=document.getElementById("products"),x=document.getElementById("pages-panel"),j=document.getElementById("previous"),J=document.getElementById("next"),U=document.getElementById("nothing-found");async function p(){const t=u(),e=await bt(t);e.length>0?(yt(),$t(e)):Lt(),t.page===1?q():rt(),t.page===g()?z():lt(),g()<=1?x.classList.add("display-none"):(x.classList.remove("display-none"),it(t))}async function bt(t){const e=await et(t);return ct(e.data.totalPages),e.data.results}function yt(){U.classList.add("display-none"),j.classList.remove("display-none"),J.classList.remove("display-none")}function Lt(){k.innerHTML="",U.classList.remove("display-none"),j.classList.add("display-none"),J.classList.add("display-none")}function G(){const t=ht.value.trim();l({...u(),keyword:t,page:1}),p()}function $t(t){const e=[];for(const n of t){const c=document.createElement("li");c.className="products-item",c.setAttribute("data-product-id",n._id);let i=`data-product-id="${n._id}"`,d=N+"#icon-shopping-cart",T="";L().includes(n._id)&&(i="",d=N+"#icon-check",T="stroke"),c.innerHTML=`
    ...
    `,c.removeEventListener("click",a),c.classList.add("stroke")}const o=document.querySelectorAll("li.products-item");for(const n of o)n.addEventListener("click",c=>{const i=c.target.nodeName.toLowerCase();i==="button"||i==="svg"||i==="use"||C(n.getAttribute("data-product-id")).then(()=>{E()})})}f();st().then(()=>{R(),p()});const K=document.getElementById("filters-form"),wt=document.getElementById("category-select"),I=document.getElementById("categories");K.addEventListener("submit",Q);K.addEventListener("input",Q);async function Q(t){t.preventDefault(),at(1),G()}wt.addEventListener("mousedown",Pt);function Pt(t){t.preventDefault(),I.classList.toggle("display-none")}I.addEventListener("click",kt);function kt(t){const e=t.target;e.nodeName.toLowerCase()==="button"&&(l({...u(),category:e.getAttribute("data-value")}),I.classList.add("display-none"),G())}let F=null;window.addEventListener("resize",Bt);function Bt(){clearTimeout(F),F=setTimeout(()=>{R()},100)}const V=document.querySelector("ul.popular-products-list");Tt();V.addEventListener("click",Ct);async function Ct(t){let e=t.target;const s=e.nodeName.toLowerCase(),a=e;for(;e&&!e.classList.contains("popular-products-list-item");)e=e.parentElement;const o=e.dataset.id;s==="button"||s==="svg"||s==="use"?($(o),f(),It(a)):(await C(o),E())}const X="/images/svg/icons.svg#icon-check",Et="popular-product-shopping-cart-button";function It(t){for(;t&&!t.classList.contains(Et);)t=t.parentElement;t.classList.remove("svg-fill-container"),t.classList.add("svg-stroke-container"),t.firstElementChild.firstElementChild.setAttribute("href",X)}async function Tt(){const t=await St(),e=At(t);Mt(e)}function Mt(t){V.innerHTML=t}async function St(){const t="https://food-boutique.b.goit.study/api/products/popular";try{return(await B.get(t)).data}catch(e){console.log(e.message)}}function At(t){return t.map(s=>Nt(s)).join("")}function Nt(t){const e={src:t.img,alt:t.name},s={name:t.name,category:t.category,size:t.size,popularity:t.popularity};return`<li class="popular-products-list-item" data-id="${t._id}">
    ${xt(e)}
    ${Ft(s)}
    ${qt(t._id)}
